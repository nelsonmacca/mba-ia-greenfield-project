...
import { CreateVideos1782446117064 } from './migrations/1782446117064-CreateVideos';
...
describe('Database migrations (integration)', () => {
  ...
  beforeAll(async () => {
    ...
    // Drop sequentially, not in parallel: concurrent DROP TABLE ... CASCADE on
    // FK-linked tables (videos → channels → users) acquire locks in conflicting
    // orders and deadlock. Sequential drops are deadlock-free.
    for (const table of [...MANAGED_TABLES, 'migrations']) {
      await dataSource.query(`DROP TABLE IF EXISTS "${table}" CASCADE`);
    }

    // Dropping the tables does not drop the enum types the migrations create.
    // A leftover type from a prior run makes runMigrations() fail with
    // 'type "..." already exists' when the full suite runs. Drop them
    // explicitly so the migrations can recreate them.
